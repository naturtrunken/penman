...
shared_context 'Api::V1::User::Network::TargetsController#timeline shared spec' do
  describe 'GET #timeline' do
    before do
      # Create an user with a network and one target and three blocks.
      @user = FactoryBot.create(:user)
      @network = FactoryBot.create(:network, user_id: @user.id)
      @target = FactoryBot.create(:target, user_network_id: @network.id)
      1.upto(3).each { FactoryBot.create(:block, user_network_target_id: @target.id) }
    end
    ...
    describe 'as the user who created the target' do
      ...
      it 'should return three targets' do
        ...
        expect(json['timeline'].count).to eq(3)
      end
    end
