...
shared_context 'Api::V1::User::Network::Target::IdeasController#create shared spec' do
  describe 'POST #create' do
    before do
      ...
      @new_text = FactoryBot.attributes_for(:idea)[:text]
    end
    ...
    describe 'as user' do
      ...
      it 'should have been created the new attack vector' do
        expect(@target.ideas.reload.where(text: @new_text).count).to eq(1)
      end
    end
    ...
    describe 'as no user' do
      ...
      it 'should not have been created the new attack vector' do
        expect(User::Network::Target::Idea.where(text: @new_text).count).to eq(0)
      end
    end
